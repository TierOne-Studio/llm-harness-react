...
import { threeWayMerge, assertGitAvailable } from './merge.js';
...
export function update({
  ...
  const conflicts = [];
  try {
    const baseDir = baseProvider(installed.package, installed.version, tmpRoot);
    ...
    for (const { path, action } of plan) {
      ...
      if (action === ACTIONS.COPY || action === ACTIONS.RESTORE) {
        ...
        // NOTE: text merge — `git merge-file` decodes as UTF-8. The shipped
        // template is text-only (markdown/toml); a binary asset would need a
        // separate copy-or-skip path before this is reached.
