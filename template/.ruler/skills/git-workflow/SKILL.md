...
Example (feature change, commonly under `src/features/`):
```
...
A change to the app's API contract types typically pairs with a change in the sibling backend repo (see `cross-repo-workspace`) — call out that the change crosses the FE↔BE seam and reference the paired branch/PR.
